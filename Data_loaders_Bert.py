...
# Create News_dict with news_id as key and Category and Subcategory and title as value
for idx, (id, Category, SubCategory) in enumerate(zip(News.news_id, News.category, News.subcategory)):
    title_dict_train[News_vocab.lookup_indices([id])[0]] = title_token_padded['input_ids'][idx]
    title_dict_train_mask[News_vocab.lookup_indices([id])[0]] = title_token_padded['attention_mask'][idx]
...
for idx, (id, Category, SubCategory) in enumerate(zip(News_vali.news_id, News_vali.category, News_vali.subcategory)):
    title_dict_vali[News_vocab.lookup_indices([id])[0]] = title_token_padded_vali['input_ids'][idx]
    title_dict_vali_mask[News_vocab.lookup_indices([id])[0]] = title_token_padded_vali['attention_mask'][idx]
...
BatchLoader = load_batch(UserData, batch_size=2, device='cpu',train=True, shuffle=False)

User_en, Category, Subcategory, History_tensor, history_len, Category_Impressions, Subcategory_Impressions, Impressions_tensor, Impressions_len, Clicked = next(BatchLoader)
